...
public partial class AllianceYavin4UnitsTableForm : AllianceYavin4UnitsTableDesignForm
{
    #region .ctor

    public AllianceYavin4UnitsTableForm()
    {
        GameFilePath = Path.Combine(Settings.Current.GDataFolder, "CMUNYVTB.DAT");
        GameFile = DatFile.Load<CMUNYVTB>(GameFilePath);
        InitializeComponent();
    }
    ...
    private void addArmyButton_Click(object sender, EventArgs e)
    {
        if (armyComboBox.SelectedItem == null)
            return;
        var unit = new CMUNYVTB_Unit()
        {
            Field1_1 = 1,
            Field2_0 = 0,
            Unit = Identifier.ToValue(armyComboBox.Text)
        };
        var army = new CMUNYVTB_Army()
        {
            Field2_1 = 1,
            Field4_1 = 1,
            Field5_1 = 1,
            ...
            Probability = 100,
            Units = new CMUNYVTB_Unit[] { unit },
            UnitsCount = 1
        };
        var armies = new List<CMUNYVTB_Army>(GameFile.Armies);
        armies.Add(army);
    ...
    private void addUnitButton_Click(object sender, EventArgs e)
    {
        ...
        var armyId = Int32.Parse(armiesDataGridView.SelectedCells[0].RowIndex.ToString());
        var army = GameFile.Armies[armyId];
        var units = new List<CMUNYVTB_Unit>(army.Units);
        units.Add(new CMUNYVTB_Unit()
        {
            Field1_1 = 1,
            Field2_0 = 0,
